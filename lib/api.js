...
export async function fetchComponentRelations(
  { sections },
  locale,
  preview = false
) {
  ...
  await Promise.all(
    sections.map(async (slice) => {
      ...
      let componentQuery = await fetchAPI(query, {
        variables: { ...args, locale },
        preview,
      });
...
export function createPagePaths(records, context) {
  ...
  records.map((item) => {
    if (item._allSlugLocales) {
      let def = item._allSlugLocales.find(
        (item) => item.locale === process.env.DEFAULT_SITE_LOCALE
      );
