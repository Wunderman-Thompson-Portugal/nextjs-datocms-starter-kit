...
export async function pageHandler(
  ...
  const data =
    result !== null && result[resultAlias] !== null
      ? result[resultAlias]
      : null;

  const siteConfig =
    result.siteConfig !== undefined
      ? result?.siteConfig
      : await getSiteConfiguration(context.locale, preview);

  const relations = data?.sections
    ? await fetchComponentRelations(
